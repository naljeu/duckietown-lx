...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    # 1. Compute odometry-based movement (distance and rotation) using encoder ticks
    # For this, we use the robot's specifications to convert encoder ticks to movement.
    wheel_radius = robot_spec['wheel_radius']
    wheel_baseline = robot_spec['wheel_baseline']

    # Compute the distance moved by each wheel
    delta_phi_l = (2 * np.pi / robot_spec['encoder_resolution']) * left_encoder_ticks
    delta_phi_r = (2 * np.pi / robot_spec['encoder_resolution']) * right_encoder_ticks
    
    # Compute the distance moved by each wheel
    left_distance = delta_phi_l * wheel_radius
    right_distance = delta_phi_r * wheel_radius
    ...
    v = (left_distance + right_distance) / 2.0  # replace this with a function that uses the encoder
    ...
    # Adaptive Gaussian noise model
    #adaptive_cov_mask = cov_mask * (1 + abs(w) + 0.5 * v)
    #s_belief = np.zeros(belief.shape)
    #gaussian_filter(p_belief, adaptive_cov_mask, output=s_belief, mode="constant")
...
def histogram_update(belief, segments, road_spec, grid_spec):
    ...
    if measurement_likelihood is not None:
        ...
        posterior = belief * measurement_likelihood
        
        # Normalize the posterior to ensure it's a valid probability distribution
        posterior_sum = np.sum(posterior)
        if posterior_sum > 0:
            belief = posterior / posterior_sum
        else:
            belief = posterior  # If sum is zero, keep the prior belief unchanged
